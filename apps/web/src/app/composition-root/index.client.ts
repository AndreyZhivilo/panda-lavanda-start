 ...
 *
 * # Why no `createIsomorphicFn` wrapper here
 *
 * This whole module is itself client-only (`*.client.*` filename) and is only
 * reached through the `useFavorites` hook, which wraps its import in a
 * `createIsomorphicFn().client(...)` "compiler safe boundary". The server
 * build therefore drops this module entirely via dead-code elimination, so
 * the use cases' own construction never reaches the server bundle. Wrapping
 * them in `createIsomorphicFn` here would be redundant - the boundary that
 * actually protects the server is the one in the hook, not here.
 */
